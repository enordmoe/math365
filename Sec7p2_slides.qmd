...
### Example: Computing the Score CI in R 
...
To achieve margin of error $ME$, I prefer to use the simpler Wald approach and solve for $n$: 
$$
n = \left(\frac{z_{\alpha/2}}{ME}\right)^2 \hat{p} \hat{q}
$$
...
$$
n \approx \left(\frac{1}{ME}\right)^2
$$
...
To ensure a 95% confidence interval for $p$ has width $ME = 0.05$,  
...
With  $\hat{p} = 0.333$, $\hat{q} = 0.667$,  $z_{\alpha/2} = 1.96$ and  $ME = 0.05$:

$$
n = \left(\frac{1.96}{.05}\right)^2 (.333)(.667) \approx \boxed{342}
$$
