...
sim_diff_means <- function(n = 30, mu1 = 5, mu2 = 5, sigma1 = 1, sigma2 = 1, reps = 10000) {
  ...
  for (i in 1:reps) {
    ...
    if (ci[1] <= mu1 - mu2 && ci[2] >= mu1 - mu2) count_in_ci <- count_in_ci + 1
    z <- est / se
    pval <- 2*(1 - pnorm(abs(z)))
    if (pval < 0.05) rejections <- rejections + 1
  }
  list(CI_coverage = count_in_ci / reps, Rejection_rate = rejections / reps)
}
sim_diff_means()
```
