...
-   **Test statistic:** a function of the sample data that summarizes
    the evidence relevant to a specific statistical hypothesis.

------------------------------------------------------------------------

## Example: Spinning a Penny  ![](https://www.usmint.gov/learn/coins-and-medals/circulating-coins/penny/_jcr_content/root/container_1426747781/imagegallerypdp/item_1740691270883.coreimg.jpeg/1765643036176/semiq-penny-obverse-unc-p.jpeg){width="27"}
...
> **Definition:** The P-value is the probability, calculated assuming
> that $H_0$ is true, of obtaining a value of the test statistic **at
> least as contradictory** to $H_0$ as what we observed.
...
::: fragment
In plain terms, the P-value tells us how surprising our data would be
**if** $H_0$ were true.
:::
...
> A small P-value means the observed outcome is **unlikely** under $H_0$
> and suggests support for $H_a$.

> A large P-value means the outcome is **plausible** under $H_0$ — we do
> not reject.

:test_tube: It is customary to call data **significant** when $H_0$ is
...
1.  A **test statistic**, a function of the sample data on which the
    decision (reject $H_0$ or do not reject $H_0$) is to be based;\
2.  A **rejection region**, $\cal R$, the set of all test statistic
    values for which $H_0$ will be rejected.
...
|              | False positive\       | True positive (Power)\ |
...
-   Suppose we set: - $\alpha = 0.05$ $\Rightarrow$ Rejection region:
    $X \leq 6$

-   If $X \leq 6$, we **reject** $H_0$. Otherwise, we **fail to reject**
    $H_0$.

Question 1: What is the actual Type I error probability? <br> <br>

::: fragment
Question 2: What is the Type II error probability if actually $p=0.4$?
If $p=0.3$ or $p=.2$
:::
...
-   Note: Power is greater when alternative $p'$ is further away from
    null $p=0.5$.
...
> **Proposition:**\
> Suppose an experiment and sample size are fixed, and a test statistic
> is chosen.\
> Then decreasing the size of the rejection region to obtain a smaller
> value of $\alpha$ results in a larger value of $\beta$ for any
> particular parameter value consistent with $H_a$.

:::: fragment
#### :brain: Key Ideas:

::: incremental
-   Reducing $\alpha$ → Shrinks rejection region → Harder to reject
    $H_0$.\
-   This caution **increases** the chance of a **Type II error**
    (failing to detect a true effect).\
-   There is **no free lunch**: controlling one error rate worsens the
    other.\
:::
::::

::: fragment
#### :chart_with_upwards_trend: Practical Meaning:

-   A test with $\alpha = \frac{1}{20}$ is more conservative than one
    with $\alpha = \frac{1}{10}$.\
-   Smaller $\alpha$ protects against false positives, but at the **cost
    of lower power**.\
-   Choosing $\alpha$ involves considering the **relative seriousness**
    of Type I vs Type II errors.
:::
...
> Select a significance level $\alpha$ (the desired Type I error rate).
> Then:
...
> The **P-value is the smallest significance level** $\alpha$ at which
> $H_0$ can be rejected.

::: callout-tip
Because of this, the P-value is also called the **observed significance
level** (OSL) for the data.
:::
...
+:==================================+:================================+
| Estimate a parameter              | Make a decision about a         |
|                                   | parameter                       |
+-----------------------------------+---------------------------------+
| Provide a range of plausible      | Decide whether to reject $H_0$  |
| values                            |                                 |
+-----------------------------------+---------------------------------+
