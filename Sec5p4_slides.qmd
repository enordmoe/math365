...
## :thinking: Warm-Up: What Would You Expect?

A college tracks students' daily step counts for a fitness study. The distribution of steps is **right-skewed** with $\mu = 8{,}000$ steps and $\sigma = 3{,}000$. 

::: incremental
- **You sample $n = 5$ students.** Sketch the shape you'd expect for $\overline{X}$.
- **You sample $n = 50$ students.** Does your answer change?
- What if the original distribution were symmetric?
:::

::: notes
Give students 30 seconds to commit to an answer before revealing the next bullet. The goal is to surface the intuition that sample size matters for the shape of the sampling distribution — even before the CLT is stated. Revisit these predictions after the CLT slide.
:::
...
## :tulip: Example: Hours Spent Outside  {.smaller}
...
::: {.callout-tip title="R Refresher: Computing Normal Probabilities"}
To find $P(X \leq x)$ when $X \sim N(\mu, \sigma)$:
```r
pnorm(x, mean = mu, sd = sigma)          # P(X ≤ x)
1 - pnorm(x, mean = mu, sd = sigma)      # P(X > x)
qnorm(p, mean = mu, sd = sigma)          # x such that P(X ≤ x) = p
```

For $\overline{X} \sim N\!\left(\mu,\, \sigma/\sqrt{n}\right)$, plug in `sd = sigma/sqrt(n)`.
:::
